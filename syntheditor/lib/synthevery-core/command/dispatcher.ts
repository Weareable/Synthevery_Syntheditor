...
export class CommandDispatcher {
    ...
    constructor(mesh: Mesh) {
        this.mesh = mesh;
        this.mesh.setCallback(MESH_PACKET_TYPE_COMMAND, this.handlePacket.bind(this));
    }
    ...
    getCommandHandler(nodeAddress: P2PMacAddress, create: boolean): CommandHandler | undefined {
        ...
        if (handler === undefined && create) {
            ...
            const newHandler = new CommandHandlerImpl(
                (data) => {
                    this.mesh.sendPacket(MESH_PACKET_TYPE_COMMAND, nodeAddress, data);
                },
    ...
    isAvailable(nodeAddress: P2PMacAddress): boolean {
        return this.mesh.isAvailable(nodeAddress) && this.hasHandler(nodeAddress);
    }
